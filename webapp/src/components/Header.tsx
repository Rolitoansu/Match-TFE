...
export const Header = () => {
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-black/10 bg-white/95 shadow-[0_14px_32px_-24px_rgba(15,23,42,0.45)] backdrop-blur-xl">
      <div className="mx-auto w-full max-w-7xl px-4 py-2.5 sm:py-3 md:px-6 md:py-4">
        <div className="flex items-center justify-between gap-2 sm:gap-3">
          ...
          <nav className="flex-1 flex items-center justify-center gap-1 sm:gap-1.5 md:gap-2 rounded-full border border-black/5 bg-slate-100/85 p-1 sm:p-1.5 shadow-sm shadow-slate-200/70">
            {items.map((item) => {
              ...
              return (
                <button
                  ...
                  type="button"
                  className={[
                    'flex items-center justify-center gap-1.5 rounded-full px-2 sm:px-2.5 py-2 transition-all duration-200 active:scale-[0.98]',
                    'h-9 sm:h-10 md:h-auto md:px-4',
                    isActive
                ...
                >
                  ...
                  />
                  <span className={[
                    'hidden sm:inline text-[11px] sm:text-xs font-bold sm:font-black tracking-[0.02em] sm:tracking-[0.12em] transition-colors',
                    'sm:uppercase',
          ...
          <div className="shrink-0 flex items-center justify-end gap-1 sm:gap-2" ref={notificationPanelRef}>
            <label className="relative hidden sm:flex">
              ...
              <select
                ...
                className="h-10 w-auto px-8 pl-9 pr-3 rounded-full border border-black/5 bg-slate-100/85 text-xs font-bold uppercase tracking-[0.08em] text-slate-600 transition-colors hover:bg-white focus:outline-none focus:ring-2 focus:ring-primary/30"
                aria-label="Language"
              >
            ...
            <label className="relative sm:hidden shrink-0">
              <span className="sr-only">Language</span>
              <Languages className="pointer-events-none absolute left-2 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-500" />
              <select
                value={currentLanguage}
                onChange={(event) => {
                  void i18n.changeLanguage(event.target.value)
                }}
                className="h-9 w-23 min-w-23 rounded-full border border-black/5 bg-slate-100/85 pl-8 pr-3 text-[11px] font-bold uppercase tracking-[0.08em] text-slate-600 transition-colors hover:bg-white focus:outline-none focus:ring-2 focus:ring-primary/30 appearance-none cursor-pointer"
                aria-label="Language"
              >
            ...
            <button
              ...
              className="relative flex h-9 sm:h-10 w-9 sm:w-10 items-center justify-center rounded-full border border-black/5 bg-slate-100/85 text-slate-500 transition-colors hover:bg-white hover:text-foreground flex-shrink-0"
            ...
            {isNotificationsOpen && (
              <div className="absolute right-0 top-12 sm:top-14 z-40 w-[calc(100vw-2rem)] sm:max-w-sm md:w-96 rounded-2xl border border-black/10 bg-white p-3 shadow-xl shadow-slate-300/60">
                  ...
                  {!loadingNotifications && notifications.map((notification) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        className="mt-0.5 rounded-lg p-1.5 text-slate-500 transition-colors hover:bg-white/80 hover:text-slate-700 disabled:opacity-50 flex-shrink-0"
                      >
                        <Trash2 size={14} />
                      </button>
                    </div>
            ...
            <button
              ...
              className="flex h-9 sm:h-10 w-9 sm:w-10 items-center justify-center rounded-full border border-black/5 bg-slate-100/85 text-slate-500 transition-colors hover:bg-red-50 hover:text-red-500 flex-shrink-0"
            >
